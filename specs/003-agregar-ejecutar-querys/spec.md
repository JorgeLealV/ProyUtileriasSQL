...
**Independent Test**: Se puede probar de forma aislada seleccionando un directorio con archivos `.sql` y verificando que aparecen en la lista izquierda, y que la ruta queda persistida en `ConfInsert.txt`.
...
2. **Given** existe una entrada `02|DirEnt|<Directorio>` válida en `ConfInsert.txt`, **When** el usuario abre la pestaña "Ejecutar Querys", **Then** la ruta se restaura en el text box, los archivos `.sql` del directorio se distribuyen correctamente entre ambas listas según la entrada `02|Querys|...`.
3. **Given** la entrada `02|DirEnt|<Directorio>` apunta a un directorio que ya no existe, **When** el usuario abre la pestaña, **Then** las entradas `02|DirEnt` y `02|Querys` se eliminan de `ConfInsert.txt` y ambas listas quedan vacías.
...
El usuario mueve archivos `.sql` entre la lista "Querys Disponibles" y "Querys Seleccionados" usando los botones Agregar, Agregar Todos, Quitar y Quitar Todos. Cada operación actualiza `ConfInsert.txt`.

**Why this priority**: Define qué archivos serán ejecutados. Sin esta selección no hay ejecución posible.

**Independent Test**: Se puede probar con un directorio ya seleccionado, moviendo elementos entre listas y verificando que `ConfInsert.txt` se actualiza correctamente tras cada operación.

**Acceptance Scenarios**:

1. **Given** hay un elemento seleccionado en "Querys Disponibles", **When** el usuario presiona "Agregar", **Then** el elemento se mueve a "Querys Seleccionados" y se actualiza `02|Querys|...` en `ConfInsert.txt`.
...
**Independent Test**: Se puede probar habilitando el checkbox "Crear Log", escribiendo un nombre en el text box, guardando, y verificando que `02|NomLog|...` aparece en `ConfInsert.txt`.
...
2. **Given** el text box de nombre de log tiene contenido y el checkbox está marcado, **When** el usuario presiona "Guardar nombre", **Then** se guarda `02|NomLog|<nombre>` en `ConfInsert.txt`.
...
- **FR-001**: Al seleccionar la pestaña "Ejecutar Querys", el sistema DEBE leer `ConfInsert.txt` y restaurar la configuración previa (directorio, querys seleccionados, nombre de log) si existe y es válida.
- **FR-002**: El sistema DEBE verificar la existencia del directorio guardado en `02|DirEnt|...`; si no existe, DEBE eliminar las entradas `02|DirEnt` y `02|Querys` de `ConfInsert.txt`.
...
- **FR-004**: Al seleccionar un nuevo directorio, el sistema DEBE actualizar/crear la entrada `02|DirEnt|<Directorio>` en `ConfInsert.txt` y limpiar ambas listas.
...
- **FR-007**: Cada operación de los cuatro botones de gestión DEBE actualizar la entrada `02|Querys|<Query1>, <Query2>, ...` en `ConfInsert.txt` sin repeticiones, almacenando los nombres sin la extensión `.sql`.
...
- **FR-010**: El botón "Guardar nombre" DEBE guardar la entrada `02|NomLog|<NombreArchLog>` en `ConfInsert.txt` solo si el text box tiene contenido.
- **FR-011**: El botón "Limpiar configuración" DEBE borrar ambas listas, el text box de directorio, desmarcar ambos checkboxes, limpiar y deshabilitar el text box de log, y eliminar las entradas `02|DirEnt`, `02|Querys` y `02|NomLog` de `ConfInsert.txt`.
...
- **FR-020**: Al restaurar configuración, si existe `02|NomLog|<NombreArchLog>` en `ConfInsert.txt`, DEBE mostrarse el nombre en el text box de log.

### Key Entities

- **ConfInsert.txt**: Archivo de configuración persistente con entradas clave-valor en formato `XX|Clave|Valor`. Las entradas de esta funcionalidad usan el prefijo `02`.
...
- **Archivo SQL**: Archivo con extensión `.sql` que contiene instrucciones SQL a ejecutar. Se identifica por su nombre sin extensión en `ConfInsert.txt`.
...
- **SC-003**: Cada operación de gestión de listas (Agregar/Quitar/Todos) refleja cambios en `ConfInsert.txt` de forma inmediata (en la misma operación, sin pasos adicionales).
...
- Los checkboxes "Crear Log de Operación" y "Permitir ejecución de Operaciones válidas" inician siempre desmarcados (no se persisten en `ConfInsert.txt`).
